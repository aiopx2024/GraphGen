...
def log_llm_call_once(call_location: str, prompt: str, context: str = None, is_before: bool = True):
    ...
    Args:
        call_location (str): 调用位置标识，如 'kg_summarization'
...
async def _handle_kg_summary(
    ...
    # 调用前记录日志（仅第一次）
    log_llm_call_once("kg_summarization", prompt, is_before=True)
    
    new_description = await llm_client.generate_answer(prompt)
    
    # 调用后记录日志（仅第一次）
    log_llm_call_once("kg_summarization", prompt, new_description, is_before=False)
